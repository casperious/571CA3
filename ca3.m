...
R=64;%<you need to decide the radius of notch filter circular support>;
x3=notch_filter(y,R);
%<compare the damaged and denoised images y and x3 side-by-side>
%imshowpair(y,x3,'montage');
imshow([y x3],[]);
%Looks less noisy, but not as clear as the one in demo_denoise
...
X1=fft2(x1);%<apply fft2 to image x>;
...
%    here?}
% To ensure that the calculations are applied on the correct values in the
% MxN matrix
...
imshow(abs(z1),[]);
% z1 does look like y, except for the top and left boundaries.
...
% The low frequency region is in the middle . fftshift re-arranges the
% quadrants of the fft so that the low frequency regions are in the middle.
% learn how to apply 2D discrete cosine transform to an image
% > help dct2
Y2=dct2(x2);
imshow(log(abs(Y2)+1),[]);
%{where is the low-frequency region in Y2?}
%The low frequency region in Y2 is in the top left of the image.
